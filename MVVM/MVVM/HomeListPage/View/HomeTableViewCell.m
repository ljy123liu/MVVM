...
- (UIImageView *)newsImageView{
    if (!_newsImageView) {
        _newsImageView=[[UIImageView alloc]initWithFrame:CGRectMake(10.0, 10.0, 85.0, 60.0)];
    }
...
- (UILabel *)newsSourceLabel{
    if (!_newsSourceLabel) {
        _newsSourceLabel=[[UILabel alloc]initWithFrame:CGRectMake(self.newsTitleLabel.x, 55.0, self.newsTitleLabel.width/2, 15.0)];
...
- (UILabel *)newsTypeLabel{
    if (!_newsTypeLabel) {
        _newsTypeLabel=[[UILabel alloc]initWithFrame:CGRectMake(self.newsImageView.x+self.newsImageView.width+self.newsTitleLabel.width/2+10.0, self.newsSourceLabel.y, self.newsSourceLabel.width, self.newsSourceLabel.height)];
...
- (void)loadViewData{
    //图片
    [self.newsImageView sd_setImageWithURL:[NSURL URLWithString:self.data.newsImage] placeholderImage:[UIImage imageNamed:@"tu_empty"]];
    //标题
    [self.newsTitleLabel setText:self.data.newsTitle];
    //来源
    [self.newsSourceLabel setText:self.data.newsSource];
    //类型
    [self.newsTypeLabel setText:self.data.newsTypeName];
}
